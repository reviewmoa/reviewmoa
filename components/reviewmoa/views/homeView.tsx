...
import { missions, progress, tagRank } from "../data";
import { accentOf, cx } from "../utils";
import { HeroStat, Panel } from "../common";
    ...
    <div className="view active">
      <div className="page">
        <section className="hero">
          ...
          <p>
            우아한테크코스 미션 PR 리뷰를 재사용 가능한 학습 규칙카드로
            정리했어요. 미션별·요청자별·카테고리별로 탐색해보세요.
          </p>
        ...
        <button className="random-teaser home-random" type="button" onClick={() => go("random")}>
          ...
          <div className="random-teaser-sum">
            도메인이 ResultSet 같은 기술 타입을 알게 되면, 나중에 JDBC를 걷어낼
            때 도메인까지 같이 고쳐야 하거든요.
          </div>
        ...
        <div className="home-grid">
          <Panel title="태그 랭킹" more="전체 보기 →" onMore={() => go("tagrank")}>
          ...
          <Panel title="카테고리 분포" more="전체 보기 →" onMore={() => go("categories")}>
            <div className="cat-dist">
              {catCounts.map(([cat, count]) => (
                <button key={cat} className="cat-dist-row" type="button" onClick={() => openCategoryCards(cat)}>
        ...
        <div className="home-grid home-grid-even">
          <Panel title="발전률 랭킹 미리보기" more="전체 보기 →" onMore={() => go("progress")}>
            <div className="prog-list">
              {progress.all.slice(0, 5).map(([id, tags, , total], index) => (
                <button key={id} className="prog-row" type="button" onClick={() => go("progress")}>
                  ...
                  <span className="prog-info">
                    ...
                    <span className="prog-meta">distinct 태그 {tags} · 카드 {total}</span>
                  </span>
          ...
          <Panel title="미션별 카드 수" more="전체 보기 →" onMore={() => go("missions")}>
            <div className="cat-dist">
              {missions.map((mission) => (
                <button key={mission.id} className="cat-dist-row" type="button" onClick={() => openMission(mission.id)}>
                  <span className="cat-dist-top">
