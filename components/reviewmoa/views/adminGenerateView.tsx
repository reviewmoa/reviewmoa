...
export function AdminGenerateView({ openJobs }: { openJobs: () => void }) {
  return (
    <>
      <div className="admin-title">규칙카드 생성</div>
      <div className="admin-sub">
        미션과 PR 번호 범위를 지정하면 서버가 리뷰를 수집하고 AI가 카드를
        만들어요. 요청 후에는 작업 관리에서 진행 상황을 확인해요.
      </div>
      <div className="form-card">
        ...
        <button className="btn-primary" type="button" onClick={openJobs}>
          <ArrowIcon />
          생성 요청 보내기
        </button>
        <p className="admin-help">
          요청을 보내면 작업 관리 화면으로 이동해요. 생성은 백그라운드에서
          진행되고, 완료되면 상태가 업데이트돼요.
        </p>
      </div>
    </>
  );
