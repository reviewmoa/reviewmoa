      ...
      <div className="page">
        ...
        <div className="rank-toggle">
          {[
            ...
            <button key={value} className={cx("rank-toggle-btn", scope === value && "on")} type="button" onClick={() => setScope(value)}>
              {label}
            </button>
          ))}
        </div>
        <div className="lb-table">
          ...
          {data.map(([id, tagCount, ratio, total], index) => (
            <button key={id} className="lb-row" type="button" onClick={() => showToast(`@${id} 카드 보기는 데모에서 생략`)}>
