...
import { cards, missions } from "../data";
import { categoryStyle } from "../utils";
import { GithubIcon, ShareIcon } from "../icons";
import { CardSection, CodeBlock, Crumb, RailRow } from "../common";
      ...
      <div className="page page-narrow">
        <Crumb
          items={[
            ["홈", () => go("home")],
            ["미션", () => go("missions")],
            [mission.name, () => openMission(mission.id)],
            [card.title]
          ]}
        />
        <div className="detail-layout">
          ...
          <aside className="detail-rail">
            <div className="rail-card">
              ...
              <RailRow label="리뷰어" value={card.reviewers.map((reviewer) => `@${reviewer}`).join(", ")} mono />
            ...
            <div className="rail-card">
              ...
              {related.map((item) => (
                <button key={item.id} className="related-item" type="button" onClick={() => openCard(item)}>
                  <span className="related-title">{item.title}</span>
                  <span className="related-meta">{item.cat} · @{item.requester}</span>
                </button>
              ))}
            </div>
          </aside>
